...
class Promote_controller extends Home_Core_Controller
{
    ...
    public function pricing_post()
    {
        ...
        $price_per_week = price_format_decimal($this->payment_settings->price_per_week);
        $price_per_month = price_format_decimal($this->payment_settings->price_per_month);

        $day_count = $this->input->post('day_count', true);
        $week_count = $this->input->post('week_count', true);
        ...
        if ($plan_type == "weekly") {
            $total_amount = number_format($week_count * $price_per_week, 2, ".", "") * 100;
            $purchased_plan = trans("daily_plan") . " (" . $week_count . " " . trans("days") . ")";
        }
        ...
        redirect(lang_base_url() . "promote-product/ipaymu");
    }
...
/**
    ...
    public function ipaymu()
    {
        ...
        $this->load->view('partials/_header', $data);
        $this->load->view('promote/ipaymu', $data);
        $this->load->view('partials/_footer');
    }
